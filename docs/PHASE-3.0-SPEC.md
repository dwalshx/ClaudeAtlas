...
#### Active-fork / semantic-clone detection (corrected from research findings 2026-05-16)

The earlier draft specified detecting "active forks" via GitHub's
`/compare` API: 10+ unique commits AND embedding distance > 0.1 from the
parent. This is **dead code in our pipeline** — `scripts/scrape.js:462`
and `scripts/scrape-discover-repos.js:309` skip `repo.fork === true` at
...
canonical" — is real, but the trigger is wrong. **Reframed as
...
#### Novelty scoring (corrected from research findings 2026-05-16)
...
**Why percentile, not absolute:** empirical analysis
(`.planning/phases/3.1-filter-overhaul/RESEARCH.md` §Q2) showed that
...
absolute-threshold gate (sim cutoff at the random-pair p95), the
noteworthy slice would be either everything (early catalog) or nothing
(mature catalog) — never stable.
...
**Implementation:** `scripts/enrich.js` writes the raw `novelty_score`
to every record. The percentile gate lives in the consumer code — Phase
